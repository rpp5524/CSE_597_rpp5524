...
class GatingModule(torch.nn.Module):
    def __init__(self, input_dim):
        ...
        self.fc = torch.nn.Linear(input_dim, 2)  # Output 2 weights for grid and region features
        self.softmax = torch.nn.Softmax(dim=-1)

    def forward(self, decoder_state):
        weights = self.fc(decoder_state)
        weights = self.softmax(weights)  # Normalize weights
        return weights[:, 0], weights[:, 1]  # Return grid and region weights
...
def main(gpu, config):
    ...
    gating_module = GatingModule(input_dim=config.model.decoder_dim).to(device)

    # model = Transformer(detector=detector, config=config, gating_module=gating_module)
